...
def load_colmap_data(realdir):
    ...
    points3dfile = os.path.join(realdir, 'colmap_sparse/0/points3D.bin')
    ...
    if os.path.isdir(os.path.join(realdir, 'images_colmap')):
        num_colmap_frames = len(
            glob.glob(os.path.join(realdir, 'images_colmap', '*.jpg')))
        num_data_frames = len(
            glob.glob(os.path.join(realdir, 'images', '*.png')))

        assert num_colmap_frames == save_arr.shape[0]
        np.save(os.path.join(realdir, 'poses_bounds.npy'),
                save_arr[:num_data_frames, :])
    else:
