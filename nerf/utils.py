...
class Trainer(object):
    def __init__(self,
        ...
        if self.world_size > 1:
            ...
            model_s = torch.nn.parallel.DistributedDataParallel(
                model_s, device_ids=[local_rank])
            model_d = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_d)
            model_d = torch.nn.parallel.DistributedDataParallel(
                model_d, device_ids=[local_rank])
        self.model_s = model_s
        self.model_d = model_d

        model_params = list(self.model_s.parameters()) + \
            list(self.model_d.parameters())
        ...
        if optimizer is None:
            self.optimizer = optim.Adam(
                model_params, lr=0.001, weight_decay=5e-4)  # naive adam
        else:
            self.optimizer = optimizer(self.model_s)  # TODO: add dynamics
        ...
        self.log(
            f'[INFO] #parameters: {sum([p.numel() for p in model_params if p.requires_grad])}')
    ...
    def train_gui(self, train_loader, step=16):
        ...
        if self.global_step == 0:
            # TODO: Does this have to be done for both static and dynamic?
            self.model_s.mark_untrained_grid(
                train_loader._data.poses, train_loader._data.intrinsics)
            self.model_d.mark_untrained_grid(
                train_loader._data.poses, train_loader._data.intrinsics)
        ...
        for _ in range(step):
            ...
            if self.model_s.cuda_ray and self.global_step % self.opt.update_extra_interval == 0:
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    self.model_s.update_extra_state()
            if self.model_d.cuda_ray and self.global_step % self.opt.update_extra_interval == 0:
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    self.model_d.update_extra_state()
