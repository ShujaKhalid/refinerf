...
then
	...
	then
		...
		for i in $DATASET_PATH/$FILENAME/images/*.png ; do convert "$i" "${i%.*}.jpg" ; done
		cp -pr $DATASET_PATH/$FILENAME/images/*.jpg $DATASET_PATH/train
		cp -pr $DATASET_PATH/$FILENAME/images/*.jpg $DATASET_PATH/images_colmap
	else
		mkdir -p $DATASET_PATH/images_colmap
