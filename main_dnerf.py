...
if __name__ == '__main__':
    ...
    if opt.basis:
        assert opt.basis, "DynamicNeRF implementation has broken this functionality :("
        ...
        from dnerf.network_s import NeRFNetworkStatic
        from dnerf.network_d import NeRFNetworkDynamic

    print(opt)

    seed_everything(opt.seed)

    model_s = NeRFNetworkStatic(
        bound=opt.bound,
    ...
    if opt.test:
        ...
        trainer = Trainer('ngp', opt, (model_s, model_d), device=device, workspace=opt.workspace,
        ...
        def optimizer(model): return torch.optim.Adam(
            model.get_params(opt.lr, opt.lr_net), betas=(0.9, 0.99), eps=1e-15)
        ...
        trainer = Trainer('ngp', opt, (model_s, model_d), device=device, workspace=opt.workspace, optimizer=optimizer, criterion=criterion, ema_decay=0.95,
                          fp16=opt.fp16, lr_scheduler=scheduler, scheduler_update_every_step=True, metrics=[PSNRMeter()], use_checkpoint=opt.ckpt, eval_interval=50)
